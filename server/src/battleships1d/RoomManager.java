...
public class RoomManager {
	...
	public static boolean userInRoom(String username) {
		for (Room r : rooms.values()) {
			if (r.getHost().getUsername().equals(username))
				return true;
			if (r.getGuest() != null
	...
	/**
	 * Generate a random number for the room name;
	 * 
	 * @return
	 */
	...
	public static void handleCommand(Command cmd, Connection connection) {
		...
		if (cmd.getParameters()[0].equals("List")) {
			...
			for (Room r : rooms.values()) {
				returnString += "::" + r.getRoomID() + "::"
						+ r.getHost().getUsername() + "::" + r.getPassword();
			}
			...
			if (cmd.getParameters().length == 1)
				newRoom = hostRoom(connection.getUser(), "");
			else
				newRoom = hostRoom(connection.getUser(), cmd.getParameters()[1]);
			connection.writeLine("Room::Create::" + newRoom.getRoomID());
		} else if (cmd.getParameters()[0].equals("Close")) {
			try {
				if (cmd.getParameters().length != 2)
					return;
				if (connection.getUser().getUsername()
						.equals(rooms.get(cmd.getParameters()[1]).getHost())) {
					connection.writeLine("Room::Close::Error::UserNotHost");
					return;
				}
				if (!roomExists(cmd.getParameters()[1]))
					return;
				if (rooms.get(cmd.getParameters()[1]).getGuest() != null) {
					rooms.get(cmd.getParameters()[1]).getGuest()
							.getConnection().writeLine("Room::Close::Success");
				}
			...
			connection.writeLine("Room::PlayedJoined::"
			...
			if (!room.getGuest().getUsername()
					.equals(connection.getUser().getUsername())) {
